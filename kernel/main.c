	...
	#define V_COMMIT_HASH "78c6e35"
#endif

#ifndef V_CURRENT_COMMIT_HASH
	#define V_CURRENT_COMMIT_HASH "78c6e35"
#endif
...
typedef struct graphic__FrameBufferConfig graphic__FrameBufferConfig;
typedef struct graphic__PixelColor graphic__PixelColor;
typedef struct bitfield__BitField bitfield__BitField;
typedef struct console__Console console__Console;
typedef struct mm__MemoryMap mm__MemoryMap;
typedef struct mm__MemoryDescriptor mm__MemoryDescriptor;
typedef struct mm__SegmentSettings mm__SegmentSettings;
typedef struct mm__SegmentDescriptorConfig mm__SegmentDescriptorConfig;
...
// added by module `graphic`:

#if defined(__has_include)

#if __has_include("graphic/font.h")
#include "graphic/font.h"
#else
#error VERROR_MESSAGE Header file "graphic/font.h", needed for module `graphic` was not found. Please install the corresponding development headers.
#endif
...
#if __has_include("asmfunc.h")
#include "asmfunc.h"
#else
#error VERROR_MESSAGE Header file "asmfunc.h", needed for module `mm` was not found. Please install the corresponding development headers.
#endif
...
#if __has_include("newlib_support.h")
#include "newlib_support.h"
#else
#error VERROR_MESSAGE Header file "newlib_support.h", needed for module `main` was not found. Please install the corresponding development headers.
#endif
...
#include "asmfunc.h"
#else
#error VERROR_MESSAGE Header file "asmfunc.h", needed for module `main` was not found. Please install the corresponding development headers.
#endif
...
typedef enum {
	graphic__PixelFormat_rgb, // 
	graphic__PixelFormat_bgr, // +1
} graphic__PixelFormat;

typedef enum {
	mm__MemoryType_reserved_memory_type, // 
	mm__MemoryType_loader_code, // +1
	mm__MemoryType_loader_data, // +2
	mm__MemoryType_boot_services_code, // +3
	mm__MemoryType_boot_services_data, // +4
	mm__MemoryType_runtime_services_code, // +5
	mm__MemoryType_runtime_services_data, // +6
	mm__MemoryType_conventional_memory, // +7
	mm__MemoryType_unusable_memory, // +8
	mm__MemoryType_acpi_reclaim_memory, // +9
	mm__MemoryType_acpi_memory_nvs, // +10
	mm__MemoryType_memory_mapped_io, // +11
	mm__MemoryType_memory_mapped_io_port_space, // +12
	mm__MemoryType_pal_code, // +13
	mm__MemoryType_persistent_memory, // +14
	mm__MemoryType_max_memory_type, // +15
} mm__MemoryType;

typedef enum {
	mm__DescriptorType_upper8bytes = 0, // 0
	mm__DescriptorType_tss_available = 9, // 9
	mm__DescriptorType_tss_busy = 11, // 11
	mm__DescriptorType_call_gate = 12, // 12
	mm__DescriptorType_interrupt_gate = 14, // 14
	mm__DescriptorType_trap_gate = 15, // 15
	mm__DescriptorType_read_write = 2, // 2
	mm__DescriptorType_execute_read = 10, // 10
} mm__DescriptorType;
...
typedef graphic__PixelColor main__Color;
typedef byte Array_fixed_byte_10 [10];
typedef byte Array_fixed_byte_16 [16];
typedef byte Array_fixed_byte_32 [32];
typedef byte Array_fixed_byte_64 [64];
typedef void (*anon_fn_placeholder_byteptr)(console__Console*,byteptr);
...
typedef string Array_fixed_string_25 [25];
typedef u64 Array_fixed_u64_3 [3];
...
typedef array Array_mm__MemoryType;
// builtin types:
//------------------ #endbuiltin
...
struct graphic__PixelColor {
	byte r;
	byte g;
	byte b;
} ;

struct graphic__FrameBufferConfig {
	byte* frame_buffer;
	u32 pixels_per_scan_line;
	u32 horizontal_resolution;
	u32 vertical_resolution;
	graphic__PixelFormat pixel_format;
} ;

struct mm__MemoryMap {
	u64 buffer_size;
	voidptr buffer;
	u64 map_size;
	u64 map_key;
...
struct mm__SegmentDescriptorConfig {
	bool is_data_segment;
	u64 limit_low;
	u64 base_low;
	u64 base_middle;
	mm__DescriptorType typ;
	u64 system_segment;
	u64 descriptor_privilege_level;
	u64 present;
	u64 limit_high;
	u64 available;
	u64 long_mode;
	u64 default_operation_size;
	u64 granularity;
	u64 base_high;
} ;

struct bitfield__BitField {
	int size;
	Array_u32 field;
} ;
...
struct console__Console {
	graphic__FrameBufferConfig* config;
	graphic__PixelColor fg_color;
	graphic__PixelColor bg_color;
	Array_fixed_string_25 buf;
	int cursor_row;
	int cursor_col;
	int rows;
	int cols;
...
struct mm__SegmentSettings {
	Array_fixed_u64_3 gdt;
} ;
...
VV_LOCAL_SYMBOL byteptr graphic__get_font(rune c);
void graphic__FrameBufferConfig_write_ascii(graphic__FrameBufferConfig* config, int x, int y, rune c, graphic__PixelColor* color);
void graphic__FrameBufferConfig_write_string(graphic__FrameBufferConfig* config, int x, int y, string s, graphic__PixelColor* color);
void graphic__FrameBufferConfig_write_pixel(graphic__FrameBufferConfig* config, int x, int y, graphic__PixelColor* c);
#define _const_bitfield__slot_size 32
bitfield__BitField bitfield__from_bytes(Array_byte input);
bitfield__BitField bitfield__from_bytes_lowest_bits_first(Array_byte input);
bitfield__BitField bitfield__from_str(string input);
string bitfield__BitField_str(bitfield__BitField input);
bitfield__BitField bitfield__new(int size);
void bitfield__BitField_free(bitfield__BitField* instance);
int bitfield__BitField_get_bit(bitfield__BitField instance, int bitnr);
void bitfield__BitField_set_bit(bitfield__BitField* instance, int bitnr);
void bitfield__BitField_clear_bit(bitfield__BitField* instance, int bitnr);
u64 bitfield__BitField_extract(bitfield__BitField instance, int start, int len);
u64 bitfield__BitField_extract_lowest_bits_first(bitfield__BitField instance, int start, int len);
void bitfield__BitField_insert_lowest_bits_first_T_u64(bitfield__BitField* instance, int start, int len, u64 _value);
void bitfield__BitField_insert_lowest_bits_first_T_int(bitfield__BitField* instance, int start, int len, int _value);
void bitfield__BitField_set_all(bitfield__BitField* instance);
void bitfield__BitField_clear_all(bitfield__BitField* instance);
void bitfield__BitField_toggle_bit(bitfield__BitField* instance, int bitnr);
bitfield__BitField bitfield__bf_and(bitfield__BitField input1, bitfield__BitField input2);
bitfield__BitField bitfield__bf_not(bitfield__BitField input);
bitfield__BitField bitfield__bf_or(bitfield__BitField input1, bitfield__BitField input2);
bitfield__BitField bitfield__bf_xor(bitfield__BitField input1, bitfield__BitField input2);
bitfield__BitField bitfield__join(bitfield__BitField input1, bitfield__BitField input2);
int bitfield__BitField_get_size(bitfield__BitField instance);
bitfield__BitField bitfield__BitField_clone(bitfield__BitField instance);
bool bitfield__BitField_cmp(bitfield__BitField instance, bitfield__BitField input);
int bitfield__BitField_pop_count(bitfield__BitField instance);
int bitfield__hamming(bitfield__BitField input1, bitfield__BitField input2);
int bitfield__BitField_pos(bitfield__BitField haystack, bitfield__BitField needle);
bitfield__BitField bitfield__BitField_slice(bitfield__BitField input, int _start, int _end);
bitfield__BitField bitfield__BitField_reverse(bitfield__BitField instance);
void bitfield__BitField_resize(bitfield__BitField* instance, int new_size);
bitfield__BitField bitfield__BitField_rotate(bitfield__BitField instance, int offset);
VV_LOCAL_SYMBOL void bitfield__BitField_clear_tail(bitfield__BitField* instance);
VV_LOCAL_SYMBOL u32 bitfield__bitmask(int bitnr);
VV_LOCAL_SYMBOL int bitfield__bitslot(int size);
VV_LOCAL_SYMBOL int bitfield__min(int input1, int input2);
VV_LOCAL_SYMBOL int bitfield__zbitnslots(int length);
console__Console console__new_console(graphic__FrameBufferConfig* config, graphic__PixelColor fg_color, graphic__PixelColor bg_color);
VV_LOCAL_SYMBOL void console__Console_new_line(console__Console* c);
void console__Console_put_string(console__Console* c, string s);
bool mm__MemoryType_is_available(mm__MemoryType t);
VV_LOCAL_SYMBOL u64 mm__segment_descriptor_factory(u64 val, mm__SegmentDescriptorConfig s);
VV_LOCAL_SYMBOL u64 mm__set_code_segment(mm__DescriptorType typ, u32 descriptor_privilege_level, u32 base, u32 limit);
VV_LOCAL_SYMBOL u64 mm__set_data_segment(mm__DescriptorType typ, u32 descriptor_privilege_level, u32 base, u32 limit);
void mm__SegmentSettings_setup_segments(mm__SegmentSettings* s);
VV_LOCAL_SYMBOL void main__halt();
VV_LOCAL_SYMBOL void main__console_print_handler(console__Console* c, byteptr s);
VV_LOCAL_SYMBOL void main__kernel_main(graphic__FrameBufferConfig* frame_buffer_config_ref, mm__MemoryMap* memory_map_ref);
...
VV_LOCAL_SYMBOL voidptr map_get_and_set_1(map* m, voidptr key, voidptr zero) {
	...
	if (!(false)) {
		...
		v_assert_meta_info__t69.fpath = _SLIT("/home/lemon/git/v/vlib/builtin/map.v");
		v_assert_meta_info__t69.line_nr = 574;
...
VV_LOCAL_SYMBOL byteptr graphic__get_font(rune c) {
	u32 index = 16 * ((u32)(c));
	if (index >= ((u32)(&_binary_hankaku_bin_size))) {
		 byteptr _t101 = ((byteptr)(0));
		// autofree_scope_vars(pos=144 line_nr=7 scope.pos=141 scope.end_pos=164)
		// af parent scope:
		// var "c" var.pos=55 var.line_nr=4
		// var "index" var.pos=74 var.line_nr=5
		// af parent scope:
		// af parent scope:
		return _t101;
	}
	 byteptr _t102 = &_binary_hankaku_bin_start + index;
	// autofree_scope_vars(pos=166 line_nr=9 scope.pos=46 scope.end_pos=222)
	// var "c" var.pos=55 var.line_nr=4
	// var "index" var.pos=74 var.line_nr=5
	// af parent scope:
	// af parent scope:
...
void graphic__FrameBufferConfig_write_ascii(graphic__FrameBufferConfig* config, int x, int y, rune c, graphic__PixelColor* color) {
	byteptr font = graphic__get_font(c);
	if (font == ((byteptr)(0))) {
		return;
	}
	for (int dy = 0; dy < 16; ++dy) {
		for (int dx = 0; dx < 8; ++dx) {
			if (((font[dy] << dx) & 0x80) != 0) {
				graphic__FrameBufferConfig_write_pixel(config, x + dx, y + dy, color);
			}
		}
	}
}
...
void graphic__FrameBufferConfig_write_string(graphic__FrameBufferConfig* config, int x, int y, string s, graphic__PixelColor* color) {
	for (int i = 0; i < s.len; ++i) {
		byte c = s.str[i];
		graphic__FrameBufferConfig_write_ascii(config, x + 8 * i, y, c, color);
	}
}

void graphic__FrameBufferConfig_write_pixel(graphic__FrameBufferConfig* config, int x, int y, graphic__PixelColor* c) {
	int pixel_position = ((int)(config->pixels_per_scan_line)) * y + x;
	{ // Unsafe block
		byte* p = &config->frame_buffer[4 * pixel_position];

		if (config->pixel_format == (graphic__PixelFormat_rgb)) {
			p[0] = c->r;
			p[1] = c->g;
			p[2] = c->b;
		}
		else if (config->pixel_format == (graphic__PixelFormat_bgr)) {
			p[0] = c->b;
			p[1] = c->g;
			p[2] = c->r;
		};
...
bitfield__BitField bitfield__from_bytes(Array_byte input) {
	bitfield__BitField output = bitfield__new(input.len * 8);
	// FOR IN array
	for (int i = 0; i < input.len; ++i) {
		byte b = ((byte*)input.data)[i];
		byte ob = ((byte)(0));
		if ((b & 0b10000000) > 0) {
			ob |= 0b00000001;
		}
		...
		(*(u32*)/*ee elem_typ */array_get(output.field, i / 4)) |= ((u32)(ob)) << ((i % 4) * 8);
	}
	// autofree_scope_vars(pos=1224 line_nr=58 scope.pos=660 scope.end_pos=1239)
	// var "input" var.pos=671 var.line_nr=28
	// var "output" var.pos=701 var.line_nr=29
	// af parent scope:
	// af parent scope:
	return output;
}
...
bitfield__BitField bitfield__from_bytes_lowest_bits_first(Array_byte input) {
	...
	for (int i = 0; i < input.len; ++i) {
		byte b = ((byte*)input.data)[i];
		(*(u32*)/*ee elem_typ */array_get(output.field, i / 4)) |= ((u32)(b)) << ((i % 4) * 8);
	}
	// autofree_scope_vars(pos=1519 line_nr=68 scope.pos=1358 scope.end_pos=1534)
	// var "input" var.pos=1387 var.line_nr=63
	// var "output" var.pos=1417 var.line_nr=64
	// af parent scope:
	// af parent scope:
	return output;
}
...
bitfield__BitField bitfield__from_str(string input) {
	bitfield__BitField output = bitfield__new(input.len);
	for (int i = 0; i < input.len; ++i) {
		if (string_at(input, i) != L'0') {
			bitfield__BitField_set_bit(&output, i);
		}
	}
	// autofree_scope_vars(pos=1815 line_nr=80 scope.pos=1673 scope.end_pos=1830)
	// var "input" var.pos=1682 var.line_nr=73
	// var "output" var.pos=1712 var.line_nr=74
	// af parent scope:
	// af parent scope:
	return output;
}
...
string bitfield__BitField_str(bitfield__BitField input) {
	...
	for (int i = 0; i < input.size; ++i) {
		if (bitfield__BitField_get_bit(input, i) == 1) {
			output = string_add(output, _SLIT("1"));
		} else {
			output = string_add(output, _SLIT("0"));
		}
	}
	// autofree_scope_vars(pos=2110 line_nr=94 scope.pos=1935 scope.end_pos=2125)
	// var "input" var.pos=1936 var.line_nr=85
	// var "output" var.pos=1972 var.line_nr=86
	// str literal
	// af parent scope:
	// af parent scope:
	return output;
...
bitfield__BitField bitfield__new(int size) {
	bitfield__BitField output = (bitfield__BitField){.size = size,.field = __new_array_with_default(bitfield__zbitnslots(size), 0, sizeof(u32), 0),};
	// autofree_scope_vars(pos=2363 line_nr=104 scope.pos=2204 scope.end_pos=2378)
	// var "size" var.pos=2208 var.line_nr=98
	// var "output" var.pos=2230 var.line_nr=99
	// af parent scope:
	// af parent scope:
	return output;
}
...
// Attr: [unsafe]
void bitfield__BitField_free(bitfield__BitField* instance) {
	array_free(&instance->field);
}

int bitfield__BitField_get_bit(bitfield__BitField instance, int bitnr) {
	if (bitnr >= instance.size) {
		 int _t103 = 0;
		// autofree_scope_vars(pos=2682 line_nr=118 scope.pos=2679 scope.end_pos=2693)
		// af parent scope:
		// var "instance" var.pos=2608 var.line_nr=116
		// var "bitnr" var.pos=2635 var.line_nr=116
		// af parent scope:
		// af parent scope:
		return _t103;
	}
	 int _t104 = ((int)((((*(u32*)/*ee elem_typ */array_get(instance.field, bitfield__bitslot(bitnr))) >> (bitnr % _const_bitfield__slot_size)) & ((u32)(1U)))));
	// autofree_scope_vars(pos=2695 line_nr=120 scope.pos=2607 scope.end_pos=2782)
	// var "instance" var.pos=2608 var.line_nr=116
	// var "bitnr" var.pos=2635 var.line_nr=116
	// af parent scope:
	// af parent scope:
...
void bitfield__BitField_set_bit(bitfield__BitField* instance, int bitnr) {
	...
	(*(u32*)/*ee elem_typ */array_get(instance->field, bitfield__bitslot(bitnr))) |= bitfield__bitmask(bitnr);
}
...
u64 bitfield__BitField_extract(bitfield__BitField instance, int start, int len) {
	if (start < 0) {
		 u64 _t105 = 0;
		// autofree_scope_vars(pos=3427 line_nr=145 scope.pos=3424 scope.end_pos=3438)
		// af parent scope:
		// var "instance" var.pos=3346 var.line_nr=142
		// var "start" var.pos=3373 var.line_nr=142
		// var "len" var.pos=3384 var.line_nr=142
		// var "output" var.pos=3445 var.line_nr=147
		// af parent scope:
		// af parent scope:
		return _t105;
	}
	u64 output = ((u64)(0U));
	for (int i = 0; i < len; ++i) {
		output |= ((u64)(bitfield__BitField_get_bit(instance, start + len - i - 1))) << i;
	}
	// autofree_scope_vars(pos=3546 line_nr=151 scope.pos=3345 scope.end_pos=3561)
	// var "instance" var.pos=3346 var.line_nr=142
	// var "start" var.pos=3373 var.line_nr=142
	// var "len" var.pos=3384 var.line_nr=142
	// var "output" var.pos=3445 var.line_nr=147
...
u64 bitfield__BitField_extract_lowest_bits_first(bitfield__BitField instance, int start, int len) {
	if (start < 0) {
		 u64 _t106 = 0;
		// autofree_scope_vars(pos=4231 line_nr=180 scope.pos=4228 scope.end_pos=4242)
		// af parent scope:
		// var "instance" var.pos=4132 var.line_nr=177
		// var "start" var.pos=4177 var.line_nr=177
		// var "len" var.pos=4188 var.line_nr=177
		// var "output" var.pos=4249 var.line_nr=182
		// af parent scope:
		// af parent scope:
		return _t106;
	}
	u64 output = ((u64)(0U));
	for (int i = 0; i < len; ++i) {
		output |= ((u64)(bitfield__BitField_get_bit(instance, start + i))) << i;
	}
	// autofree_scope_vars(pos=4340 line_nr=186 scope.pos=4131 scope.end_pos=4355)
	// var "instance" var.pos=4132 var.line_nr=177
	// var "start" var.pos=4177 var.line_nr=177
	// var "len" var.pos=4188 var.line_nr=177
	// var "output" var.pos=4249 var.line_nr=182
...
void bitfield__BitField_insert_lowest_bits_first_T_u64(bitfield__BitField* instance, int start, int len, u64 _value) {
	...
	for (int pos = start; pos < start + len; ++pos) {
		if ((value & 1) == 1) {
			bitfield__BitField_set_bit(instance, pos);
		} else {
			bitfield__BitField_clear_bit(instance, pos);
		}
		value >>= 1;
	}
}
...
void bitfield__BitField_set_all(bitfield__BitField* instance) {
	for (int i = 0; i < bitfield__zbitnslots(instance->size); ++i) {
		(*(u32*)/*ee elem_typ */array_get(instance->field, i)) = ((u32)(-1));
	}
	bitfield__BitField_clear_tail(instance);
}

void bitfield__BitField_clear_all(bitfield__BitField* instance) {
	for (int i = 0; i < bitfield__zbitnslots(instance->size); ++i) {
		(*(u32*)/*ee elem_typ */array_get(instance->field, i)) = ((u32)(0U));
	}
}
...
bitfield__BitField bitfield__bf_and(bitfield__BitField input1, bitfield__BitField input2) {
	int size = bitfield__min(input1.size, input2.size);
	int bitnslots = bitfield__zbitnslots(size);
	bitfield__BitField output = bitfield__new(size);
	for (int i = 0; i < bitnslots; ++i) {
		(*(u32*)/*ee elem_typ */array_get(output.field, i)) = ((*(u32*)/*ee elem_typ */array_get(input1.field, i)) & (*(u32*)/*ee elem_typ */array_get(input2.field, i)));
	}
	bitfield__BitField_clear_tail(&output);
	// autofree_scope_vars(pos=5857 line_nr=243 scope.pos=5605 scope.end_pos=5872)
	// var "input1" var.pos=5612 var.line_nr=235
	// var "input2" var.pos=5629 var.line_nr=235
	// var "size" var.pos=5658 var.line_nr=236
	// var "bitnslots" var.pos=5697 var.line_nr=237
	// var "output" var.pos=5732 var.line_nr=238
	// af parent scope:
	// af parent scope:
	return output;
}
...
bitfield__BitField bitfield__bf_not(bitfield__BitField input) {
	...
	bitfield__BitField output = bitfield__new(size);
	for (int i = 0; i < bitnslots; ++i) {
		(*(u32*)/*ee elem_typ */array_get(output.field, i)) = ~(*(u32*)/*ee elem_typ */array_get(input.field, i));
	}
	bitfield__BitField_clear_tail(&output);
	// autofree_scope_vars(pos=6160 line_nr=255 scope.pos=5963 scope.end_pos=6175)
	// var "input" var.pos=5970 var.line_nr=247
	// var "size" var.pos=5998 var.line_nr=248
	// var "bitnslots" var.pos=6018 var.line_nr=249
	// var "output" var.pos=6053 var.line_nr=250
	// af parent scope:
	// af parent scope:
	return output;
}
...
bitfield__BitField bitfield__bf_or(bitfield__BitField input1, bitfield__BitField input2) {
	...
	bitfield__BitField_clear_tail(&output);
	// autofree_scope_vars(pos=6634 line_nr=269 scope.pos=6383 scope.end_pos=6649)
	// var "input1" var.pos=6389 var.line_nr=261
	// var "input2" var.pos=6406 var.line_nr=261
	// var "size" var.pos=6435 var.line_nr=262
	// var "bitnslots" var.pos=6474 var.line_nr=263
	// var "output" var.pos=6509 var.line_nr=264
	// af parent scope:
	// af parent scope:
	return output;
}
...
bitfield__BitField bitfield__bf_xor(bitfield__BitField input1, bitfield__BitField input2) {
	...
	bitfield__BitField_clear_tail(&output);
	// autofree_scope_vars(pos=7110 line_nr=283 scope.pos=6858 scope.end_pos=7125)
	// var "input1" var.pos=6865 var.line_nr=275
	// var "input2" var.pos=6882 var.line_nr=275
	// var "size" var.pos=6911 var.line_nr=276
	// var "bitnslots" var.pos=6950 var.line_nr=277
	// var "output" var.pos=6985 var.line_nr=278
	// af parent scope:
	// af parent scope:
	return output;
}
...
bitfield__BitField bitfield__join(bitfield__BitField input1, bitfield__BitField input2) {
	...
	bitfield__BitField output = bitfield__new(output_size);
	for (int i = 0; i < bitfield__zbitnslots(input1.size); ++i) {
		(*(u32*)/*ee elem_typ */array_get(output.field, i)) = (*(u32*)/*ee elem_typ */array_get(input1.field, i));
	}
	int offset_bit = input1.size % _const_bitfield__slot_size;
	int offset_slot = input1.size / _const_bitfield__slot_size;
	for (int i = 0; i < bitfield__zbitnslots(input2.size); ++i) {
		(*(u32*)/*ee elem_typ */array_get(output.field, i + offset_slot)) |= ((u32)((*(u32*)/*ee elem_typ */array_get(input2.field, i)) << ((u32)(offset_bit))));
	}
	if ((output_size - 1) % _const_bitfield__slot_size < (input2.size - 1) % _const_bitfield__slot_size) {
		for (int i = 0; i < bitfield__zbitnslots(input2.size); ++i) {
			(*(u32*)/*ee elem_typ */array_get(output.field, i + offset_slot + 1)) |= ((u32)((*(u32*)/*ee elem_typ */array_get(input2.field, i)) >> ((u32)(_const_bitfield__slot_size - offset_bit))));
		}
	} else if ((output_size - 1) % _const_bitfield__slot_size > (input2.size - 1) % _const_bitfield__slot_size) {
		for (int i = 0; i < bitfield__zbitnslots(input2.size) - 1; ++i) {
			(*(u32*)/*ee elem_typ */array_get(output.field, i + offset_slot + 1)) |= ((u32)((*(u32*)/*ee elem_typ */array_get(input2.field, i)) >> ((u32)(_const_bitfield__slot_size - offset_bit))));
		}
	}
	// autofree_scope_vars(pos=8795 line_nr=322 scope.pos=7209 scope.end_pos=8810)
	// var "input1" var.pos=7214 var.line_nr=287
	// var "input2" var.pos=7231 var.line_nr=287
	// var "output_size" var.pos=7260 var.line_nr=288
	// var "output" var.pos=7306 var.line_nr=289
	// var "offset_bit" var.pos=7491 var.line_nr=295
	// var "offset_slot" var.pos=7539 var.line_nr=296
	// af parent scope:
	// af parent scope:
	return output;
}
...
int bitfield__BitField_get_size(bitfield__BitField instance) {
	 int _t107 = instance.size;
	// autofree_scope_vars(pos=8916 line_nr=327 scope.pos=8879 scope.end_pos=8938)
	// var "instance" var.pos=8880 var.line_nr=326
	// af parent scope:
	// af parent scope:
	return _t107;
}
...
bitfield__BitField bitfield__BitField_clone(bitfield__BitField instance) {
	int bitnslots = bitfield__zbitnslots(instance.size);
	bitfield__BitField output = bitfield__new(instance.size);
	for (int i = 0; i < bitnslots; ++i) {
		(*(u32*)/*ee elem_typ */array_get(output.field, i)) = (*(u32*)/*ee elem_typ */array_get(instance.field, i));
	}
	// autofree_scope_vars(pos=9169 line_nr=337 scope.pos=8988 scope.end_pos=9184)
	// var "instance" var.pos=8989 var.line_nr=331
	// var "bitnslots" var.pos=9028 var.line_nr=332
	// var "output" var.pos=9072 var.line_nr=333
	// af parent scope:
	// af parent scope:
	return output;
}
...
bool bitfield__BitField_cmp(bitfield__BitField instance, bitfield__BitField input) {
	if (instance.size != input.size) {
		 bool _t108 = false;
		// autofree_scope_vars(pos=9408 line_nr=344 scope.pos=9405 scope.end_pos=9423)
		// af parent scope:
		// var "instance" var.pos=9327 var.line_nr=342
		// var "input" var.pos=9350 var.line_nr=342
		// af parent scope:
		// af parent scope:
		return _t108;
	}
	for (int i = 0; i < bitfield__zbitnslots(instance.size); ++i) {
		if ((*(u32*)/*ee elem_typ */array_get(instance.field, i)) != (*(u32*)/*ee elem_typ */array_get(input.field, i))) {
			 bool _t109 = false;
			// autofree_scope_vars(pos=9513 line_nr=348 scope.pos=9509 scope.end_pos=9529)
			// af parent scope:
			// var "i" var.pos=9430 var.line_nr=346
			// skipping tmp var "i"
			// af parent scope:
			// var "instance" var.pos=9327 var.line_nr=342
			// var "input" var.pos=9350 var.line_nr=342
	 ...
	 bool _t110 = true;
	// autofree_scope_vars(pos=9534 line_nr=351 scope.pos=9326 scope.end_pos=9547)
	// var "instance" var.pos=9327 var.line_nr=342
	// var "input" var.pos=9350 var.line_nr=342
	// af parent scope:
	// af parent scope:
...
int bitfield__BitField_pop_count(bitfield__BitField instance) {
	...
	int bitnslots = bitfield__zbitnslots(size);
	int tail = size % _const_bitfield__slot_size;
	int count = 0;
	for (int i = 0; i < bitnslots - 1; ++i) {
		for (int j = 0; j < _const_bitfield__slot_size; ++j) {
			if ((((u32)((*(u32*)/*ee elem_typ */array_get(instance.field, i)) >> ((u32)(j)))) & ((u32)(1U))) == ((u32)(1U))) {
				count++;
			}
		}
	}
	for (int j = 0; j < tail; ++j) {
		if ((((u32)((*(u32*)/*ee elem_typ */array_get(instance.field, bitnslots - 1)) >> ((u32)(j)))) & ((u32)(1U))) == ((u32)(1U))) {
			count++;
		}
	}
	// autofree_scope_vars(pos=10028 line_nr=372 scope.pos=9622 scope.end_pos=10042)
	// var "instance" var.pos=9623 var.line_nr=355
	// var "size" var.pos=9661 var.line_nr=356
	// var "bitnslots" var.pos=9684 var.line_nr=357
	// var "tail" var.pos=9715 var.line_nr=358
	// var "count" var.pos=9754 var.line_nr=359
	// af parent scope:
	// af parent scope:
	return count;
}
...
int bitfield__hamming(bitfield__BitField input1, bitfield__BitField input2) {
	bitfield__BitField input_xored = bitfield__bf_xor(input1, input2);
	 int _t111 = bitfield__BitField_pop_count(input_xored);
	// autofree_scope_vars(pos=10204 line_nr=378 scope.pos=10117 scope.end_pos=10236)
	// var "input1" var.pos=10125 var.line_nr=376
	// var "input2" var.pos=10142 var.line_nr=376
	// var "input_xored" var.pos=10166 var.line_nr=377
	// af parent scope:
	// af parent scope:
	return _t111;
}
...
int bitfield__BitField_pos(bitfield__BitField haystack, bitfield__BitField needle) {
	int heystack_size = haystack.size;
	int needle_size = needle.size;
	int diff = heystack_size - needle_size;
	if (diff < 0) {
		 int _t112 = -2;
		// autofree_scope_vars(pos=10593 line_nr=389 scope.pos=10590 scope.end_pos=10605)
		// af parent scope:
		// var "haystack" var.pos=10380 var.line_nr=383
		// var "needle" var.pos=10403 var.line_nr=383
		// var "heystack_size" var.pos=10427 var.line_nr=384
		// var "needle_size" var.pos=10459 var.line_nr=385
		// var "diff" var.pos=10487 var.line_nr=386
		// af parent scope:
		// af parent scope:
		return _t112;
	}
	for (int i = 0; i <= diff; i++) {
		bitfield__BitField needle_candidate = bitfield__BitField_slice(haystack, i, needle_size + i);
		if (bitfield__BitField_cmp(needle_candidate, needle)) {
			// autofree_scope_vars(pos=10820 line_nr=395 scope.pos=10728 scope.end_pos=10832)
			// af parent scope:
			// var "i" var.pos=10612 var.line_nr=391
			// var "needle_candidate" var.pos=10639 var.line_nr=392
			// af parent scope:
			// var "haystack" var.pos=10380 var.line_nr=383
			// var "needle" var.pos=10403 var.line_nr=383
			// var "heystack_size" var.pos=10427 var.line_nr=384
	 ...
	 int _t113 = -1;
	// autofree_scope_vars(pos=10868 line_nr=399 scope.pos=10379 scope.end_pos=10879)
	// var "haystack" var.pos=10380 var.line_nr=383
	// var "needle" var.pos=10403 var.line_nr=383
	// var "heystack_size" var.pos=10427 var.line_nr=384
	// var "needle_size" var.pos=10459 var.line_nr=385
...
bitfield__BitField bitfield__BitField_slice(bitfield__BitField input, int _start, int _end) {
	...
	bitfield__BitField output = bitfield__new(end - start);
	int start_offset = start % _const_bitfield__slot_size;
	int end_offset = (end - 1) % _const_bitfield__slot_size;
	int start_slot = start / _const_bitfield__slot_size;
	int end_slot = (end - 1) / _const_bitfield__slot_size;
	int output_slots = bitfield__zbitnslots(end - start);
	if (output_slots > 1) {
		if (start_offset != 0) {
			for (int i = 0; i < output_slots - 1; ++i) {
				(*(u32*)/*ee elem_typ */array_get(output.field, i)) = ((u32)((*(u32*)/*ee elem_typ */array_get(input.field, start_slot + i)) >> ((u32)(start_offset))));
				(*(u32*)/*ee elem_typ */array_get(output.field, i)) = ((*(u32*)/*ee elem_typ */array_get(output.field, i)) | ((u32)((*(u32*)/*ee elem_typ */array_get(input.field, start_slot + i + 1)) << ((u32)(_const_bitfield__slot_size - start_offset)))));
			}
		} else {
			for (int i = 0; i < output_slots - 1; ++i) {
				(*(u32*)/*ee elem_typ */array_get(output.field, i)) = ((u32)((*(u32*)/*ee elem_typ */array_get(input.field, start_slot + i))));
			}
		}
	}
	if (start_offset > end_offset) {
		(*(u32*)/*ee elem_typ */array_get(output.field, (end - start - 1) / _const_bitfield__slot_size)) = ((u32)((*(u32*)/*ee elem_typ */array_get(input.field, end_slot - 1)) >> ((u32)(start_offset))));
		u32 mask = ((u32)((1 << (end_offset + 1)) - 1));
		mask = ((*(u32*)/*ee elem_typ */array_get(input.field, end_slot)) & mask);
		mask = ((u32)(mask << ((u32)(_const_bitfield__slot_size - start_offset))));
		(*(u32*)/*ee elem_typ */array_get(output.field, (end - start - 1) / _const_bitfield__slot_size)) |= mask;
	} else if (start_offset == 0) {
		u32 mask = ((u32)(0U));
		if (end_offset == _const_bitfield__slot_size - 1) {
			mask = ((u32)(-1));
		} else {
			mask = ((u32)(((u32)(1U)) << ((u32)(end_offset + 1))));
			mask = mask - ((u32)(1U));
		}
		(*(u32*)/*ee elem_typ */array_get(output.field, (end - start - 1) / _const_bitfield__slot_size)) = (((*(u32*)/*ee elem_typ */array_get(input.field, end_slot)) & mask));
	} else {
		u32 mask = ((u32)(((1 << (end_offset - start_offset + 1)) - 1) << start_offset));
		mask = ((*(u32*)/*ee elem_typ */array_get(input.field, end_slot)) & mask);
		mask = ((u32)(mask >> ((u32)(start_offset))));
		(*(u32*)/*ee elem_typ */array_get(output.field, (end - start - 1) / _const_bitfield__slot_size)) |= mask;
	}
	// autofree_scope_vars(pos=12729 line_nr=453 scope.pos=10992 scope.end_pos=12744)
	// var "input" var.pos=10993 var.line_nr=404
	// var "_start" var.pos=11015 var.line_nr=404
	// var "_end" var.pos=11027 var.line_nr=404
	// var "start" var.pos=11073 var.line_nr=406
	// var "end" var.pos=11094 var.line_nr=407
	// var "output" var.pos=11217 var.line_nr=414
	// var "start_offset" var.pos=11245 var.line_nr=415
	// var "end_offset" var.pos=11289 var.line_nr=416
	// var "start_slot" var.pos=11335 var.line_nr=417
	// var "end_slot" var.pos=11377 var.line_nr=418
	// var "output_slots" var.pos=11421 var.line_nr=419
	// af parent scope:
	// af parent scope:
	return output;
}
...
bitfield__BitField bitfield__BitField_reverse(bitfield__BitField instance) {
	...
	for (int i = 0; i < (bitnslots - 1); i++) {
		for (int j = 0; j < _const_bitfield__slot_size; ++j) {
			if ((((u32)((*(u32*)/*ee elem_typ */array_get(instance.field, i)) >> ((u32)(j)))) & ((u32)(1U))) == ((u32)(1U))) {
				bitfield__BitField_set_bit(&output, size - i * _const_bitfield__slot_size - j - 1);
			}
		}
	}
	int bits_in_last_input_slot = (size - 1) % _const_bitfield__slot_size + 1;
	for (int j = 0; j < bits_in_last_input_slot; ++j) {
		if ((((u32)((*(u32*)/*ee elem_typ */array_get(instance.field, bitnslots - 1)) >> ((u32)(j)))) & ((u32)(1U))) == ((u32)(1U))) {
			bitfield__BitField_set_bit(&output, bits_in_last_input_slot - j - 1);
		}
	}
	// autofree_scope_vars(pos=13445 line_nr=475 scope.pos=12884 scope.end_pos=13460)
	// var "instance" var.pos=12885 var.line_nr=458
	// var "size" var.pos=12926 var.line_nr=459
	// var "bitnslots" var.pos=12949 var.line_nr=460
	// var "output" var.pos=12984 var.line_nr=461
	// var "bits_in_last_input_slot" var.pos=13212 var.line_nr=469
	// af parent scope:
	// af parent scope:
	return output;
}
...
void bitfield__BitField_resize(bitfield__BitField* instance, int new_size) {
	int new_bitnslots = bitfield__zbitnslots(new_size);
	int old_size = instance->size;
	int old_bitnslots = bitfield__zbitnslots(old_size);
	Array_u32 field = __new_array_with_default(new_bitnslots, 0, sizeof(u32), 0);
	for (int i = 0; i < old_bitnslots && i < new_bitnslots; i++) {
		array_set(&field, i, &(u32[]) { (*(u32*)/*ee elem_typ */array_get(instance->field, i)) });
	}
	instance->field = array_clone(&field);
	instance->size = new_size;
	if (new_size < old_size && new_size % _const_bitfield__slot_size != 0) {
		bitfield__BitField_clear_tail(instance);
	}
}
...
bitfield__BitField bitfield__BitField_rotate(bitfield__BitField instance, int offset) {
	...
	if (offset_internal == 0) {
		// autofree_scope_vars(pos=14483 line_nr=509 scope.pos=14458 scope.end_pos=14501)
		// af parent scope:
		// var "instance" var.pos=14092 var.line_nr=496
		// var "offset" var.pos=14118 var.line_nr=496
		// var "size" var.pos=14344 var.line_nr=504
		// var "offset_internal" var.pos=14400 var.line_nr=506
		// var "first_chunk" var.pos=14576 var.line_nr=514
		// var "second_chunk" var.pos=14627 var.line_nr=515
		// var "output" var.pos=14682 var.line_nr=516
		// af parent scope:
		// af parent scope:
		return instance;
	}
	...
	bitfield__BitField first_chunk = bitfield__BitField_slice(instance, 0, offset_internal);
	bitfield__BitField second_chunk = bitfield__BitField_slice(instance, offset_internal, size);
	bitfield__BitField output = bitfield__join(second_chunk, first_chunk);
	// autofree_scope_vars(pos=14724 line_nr=517 scope.pos=14091 scope.end_pos=14739)
	// var "instance" var.pos=14092 var.line_nr=496
	// var "offset" var.pos=14118 var.line_nr=496
	// var "size" var.pos=14344 var.line_nr=504
	// var "offset_internal" var.pos=14400 var.line_nr=506
...
VV_LOCAL_SYMBOL void bitfield__BitField_clear_tail(bitfield__BitField* instance) {
	int tail = instance->size % _const_bitfield__slot_size;
	if (tail != 0) {
		u32 mask = ((u32)((1 << tail) - 1));
		(*(u32*)/*ee elem_typ */array_get(instance->field, bitfield__zbitnslots(instance->size) - 1)) = ((*(u32*)/*ee elem_typ */array_get(instance->field, bitfield__zbitnslots(instance->size) - 1)) & mask);
	}
}

VV_LOCAL_SYMBOL u32 bitfield__bitmask(int bitnr) {
	 u32 _t114 = ((u32)(((u32)(1U)) << ((u32)(bitnr % _const_bitfield__slot_size))));
	// autofree_scope_vars(pos=15261 line_nr=534 scope.pos=15236 scope.end_pos=15316)
	// var "bitnr" var.pos=15244 var.line_nr=533
	// af parent scope:
	// af parent scope:
	return _t114;
}
...
VV_LOCAL_SYMBOL int bitfield__bitslot(int size) {
	 int _t115 = size / _const_bitfield__slot_size;
	// autofree_scope_vars(pos=15428 line_nr=539 scope.pos=15404 scope.end_pos=15462)
	// var "size" var.pos=15412 var.line_nr=538
	// af parent scope:
	// af parent scope:
	return _t115;
}
...
VV_LOCAL_SYMBOL int bitfield__min(int input1, int input2) {
	if (input1 < input2) {
		// autofree_scope_vars(pos=15616 line_nr=545 scope.pos=15613 scope.end_pos=15632)
		// af parent scope:
		// var "input1" var.pos=15563 var.line_nr=543
		// var "input2" var.pos=15575 var.line_nr=543
		// af parent scope:
		// af parent scope:
		return input1;
	} else {
		// autofree_scope_vars(pos=15642 line_nr=547 scope.pos=15639 scope.end_pos=15658)
		// af parent scope:
		// var "input1" var.pos=15563 var.line_nr=543
		// var "input2" var.pos=15575 var.line_nr=543
		// af parent scope:
...
VV_LOCAL_SYMBOL int bitfield__zbitnslots(int length) {
	 int _t116 = (length - 1) / _const_bitfield__slot_size + 1;
	// autofree_scope_vars(pos=15801 line_nr=553 scope.pos=15772 scope.end_pos=15847)
	// var "length" var.pos=15783 var.line_nr=552
	// af parent scope:
	// af parent scope:
	return _t116;
}
...
console__Console console__new_console(graphic__FrameBufferConfig* config, graphic__PixelColor fg_color, graphic__PixelColor bg_color) {
	 console__Console _t117 = (console__Console){
		.config = config,
		.fg_color = fg_color,
		.bg_color = bg_color,
		.buf = {_SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 "), _SLIT("                                                                                 ")},
		.cursor_row = 0,
		.cursor_col = 0,
		.rows = 25,
		.cols = 80,
	};
	// autofree_scope_vars(pos=360 line_nr=19 scope.pos=244 scope.end_pos=580)
	// var "config" var.pos=256 var.line_nr=17
	// var "fg_color" var.pos=292 var.line_nr=18
	// var "bg_color" var.pos=321 var.line_nr=18
	// af parent scope:
	// af parent scope:
	return _t117;
}
...
VV_LOCAL_SYMBOL void console__Console_new_line(console__Console* c) {
	...
	if (c->cursor_row < c->rows - 1) {
		...
		for (int y = 0; y < 16 * c->rows; ++y) {
			for (int x = 0; x < 8 * c->cols; ++x) {
				graphic__FrameBufferConfig_write_pixel(c->config, x, y, (voidptr)&/*qq*/c->bg_color);
			}
		}
		for (int row = 0; row < c->rows - 1; ++row) {
			c->buf[v_fixed_index(row, 25)] = c->buf[v_fixed_index(row + 1, 25)];
			graphic__FrameBufferConfig_write_string(c->config, 0, 16 * row, c->buf[v_fixed_index(row, 25)], (voidptr)&/*qq*/c->fg_color);
		}
		c->buf[v_fixed_index(c->rows - 1, 25)] = _SLIT("");
	}
}

void console__Console_put_string(console__Console* c, string s) {
	for (int _t118 = 0; _t118 < s.len; ++_t118) {
		byte ch = s.str[_t118];
		if (ch == L'\n') {
			console__Console_new_line(c);
		} else if (c->cursor_col < c->cols - 1) {
			graphic__FrameBufferConfig_write_ascii(c->config, 8 * c->cursor_col, 16 * c->cursor_row, ch, (voidptr)&/*qq*/c->fg_color);
			{ // Unsafe block
				((byteptr)(c->buf[v_fixed_index(c->cursor_row, 25)].str))[c->cursor_col] = ch;
			}
			c->cursor_col++;
		}
	}
...
bool mm__MemoryType_is_available(mm__MemoryType t) {
	 bool _t119 = (t == mm__MemoryType_boot_services_code || t == mm__MemoryType_boot_services_data || t == mm__MemoryType_conventional_memory);
	// autofree_scope_vars(pos=637 line_nr=39 scope.pos=600 scope.end_pos=715)
	// var "t" var.pos=601 var.line_nr=38
	// af parent scope:
	// af parent scope:
	return _t119;
}
...
VV_LOCAL_SYMBOL u64 mm__segment_descriptor_factory(u64 val, mm__SegmentDescriptorConfig s) {
	bitfield__BitField output = bitfield__new(64);
	bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 0, 64, val);
	if (!s.is_data_segment) {
		bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 0, 16, s.limit_low);
		bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 16, 16, s.base_low);
		bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 32, 8, s.base_middle);
		bitfield__BitField_insert_lowest_bits_first_T_int(&output, 40, 4, ((int)(s.typ)));
		bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 44, 1, s.system_segment);
		bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 45, 2, s.descriptor_privilege_level);
		bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 47, 1, s.present);
		bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 48, 4, s.limit_high);
		bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 52, 1, s.available);
		bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 53, 1, s.long_mode);
		bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 55, 1, s.granularity);
	}
	bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 54, 1, s.default_operation_size);
	bitfield__BitField_insert_lowest_bits_first_T_u64(&output, 56, 8, s.base_high);
	 u64 _t120 = bitfield__BitField_extract_lowest_bits_first(output, 0, 64);
	// autofree_scope_vars(pos=1519 line_nr=59 scope.pos=608 scope.end_pos=1567)
	// var "val" var.pos=635 var.line_nr=41
	// var "s" var.pos=644 var.line_nr=41
	// var "output" var.pos=682 var.line_nr=42
	// af parent scope:
	// af parent scope:
	return _t120;
}
...
VV_LOCAL_SYMBOL u64 mm__set_code_segment(mm__DescriptorType typ, u32 descriptor_privilege_level, u32 base, u32 limit) {
	 u64 _t121 = mm__segment_descriptor_factory(0, (mm__SegmentDescriptorConfig){
	...
	// autofree_scope_vars(pos=1669 line_nr=63 scope.pos=1573 scope.end_pos=2089)
	// var "typ" var.pos=1590 var.line_nr=62
	// var "descriptor_privilege_level" var.pos=1610 var.line_nr=62
	// var "base" var.pos=1642 var.line_nr=62
	// var "limit" var.pos=1652 var.line_nr=62
	// af parent scope:
	// af parent scope:
	return _t121;
}
...
VV_LOCAL_SYMBOL u64 mm__set_data_segment(mm__DescriptorType typ, u32 descriptor_privilege_level, u32 base, u32 limit) {
	u64 desc = mm__set_code_segment(typ, descriptor_privilege_level, base, limit);
	 u64 _t122 = mm__segment_descriptor_factory(desc, (mm__SegmentDescriptorConfig){.is_data_segment = true,.limit_low = 0,.base_low = 0,.base_middle = 0,.typ = 0,.system_segment = 0,.descriptor_privilege_level = 0,.present = 0,.limit_high = 0,.available = 0,.long_mode = 0,.default_operation_size = 1,.granularity = 0,.base_high = 0,});
	// autofree_scope_vars(pos=2263 line_nr=86 scope.pos=2095 scope.end_pos=2380)
	// var "typ" var.pos=2112 var.line_nr=84
	// var "descriptor_privilege_level" var.pos=2132 var.line_nr=84
	// var "base" var.pos=2164 var.line_nr=84
	// var "limit" var.pos=2174 var.line_nr=84
	// var "desc" var.pos=2192 var.line_nr=85
	// af parent scope:
	// af parent scope:
	return _t122;
}
...
void mm__SegmentSettings_setup_segments(mm__SegmentSettings* s) {
	s->gdt[0] = 0;
	s->gdt[1] = mm__set_code_segment(mm__DescriptorType_execute_read, 0, 0, 0xfffff);
	s->gdt[2] = mm__set_data_segment(mm__DescriptorType_read_write, 0, 0, 0xfffff);
	LoadGDT(/*SizeOf*/ sizeof(Array_fixed_u64_3) - 1, *&s->gdt[0]);
}

// TypeDecl
// Attr: [noreturn]
VV_LOCAL_SYMBOL void main__halt(void) {
	for (;;) {
		__asm__ (
			"hlt ;"
			: );
...
VV_LOCAL_SYMBOL void main__console_print_handler(console__Console* c, byteptr s) {
	console__Console_put_string(c, byteptr_vstring(s));
}

VV_LOCAL_SYMBOL void main__kernel_main(graphic__FrameBufferConfig* frame_buffer_config_ref, mm__MemoryMap* memory_map_ref) {
	graphic__FrameBufferConfig frame_buffer_config = *frame_buffer_config_ref;
	mm__MemoryMap memory_map = *memory_map_ref;
	if (!(((int)(frame_buffer_config.pixel_format)) == 0 || ((int)(frame_buffer_config.pixel_format)) == 1)) {
		main__halt();
	}
	for (int x = 0; x < frame_buffer_config.horizontal_resolution; ++x) {
		for (int y = 0; y < frame_buffer_config.vertical_resolution; ++y) {
			graphic__FrameBufferConfig_write_pixel(&frame_buffer_config, x, y, (voidptr)&/*qq*/(main__Color){.r = 255,.g = 255,.b = 255,});
		}
	}
	for (int x = 0; x < 200; ++x) {
		for (int y = 0; y < 100; ++y) {
			graphic__FrameBufferConfig_write_pixel(&frame_buffer_config, 100 + x, 100 + y, (voidptr)&/*qq*/(main__Color){.r = 0,.g = 255,.b = 0,});
		}
	}
	console__Console cons = console__new_console((voidptr)&/*qq*/frame_buffer_config, (main__Color){.r = 0,.g = 0,.b = 0,}, (main__Color){.r = 255,.g = 255,.b = 255,});
	register_console_print_handler(&cons, &main__console_print_handler);
	mm__SegmentSettings segment_settings = (mm__SegmentSettings){.gdt = {0},};
	mm__SegmentSettings_setup_segments(&segment_settings);
	int kernel_cs = 1 << 3;
	int kernel_ss = 2 << 3;
	SetDSAll(0);
...
void _vinit(int ___argc, voidptr ___argv) {
	...
	// Initializations for module graphic :
	// Initializations for module bitfield :
	// Initializations for module console :
	// Initializations for module mm :
